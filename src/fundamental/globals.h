...
void initCam (Vector2 target, float rotation, float zoom) {
    ...
    logHandler(LOG_INFO, "Initialized global camera: target=(%f,%f), zoom=%d, rotation=%f", target.x, target.y, zoom, rotation);
}

void delta (void) {
    g_dt = GetFrameTime();
    logHandler(LOG_INFO, "A frame takes %f milliseconds", g_dt*1000.0f);
}
