...
void drawDialogueBox (DialogueBox dbox, float* cdown, bool* enabled) {
    if (!enabled || !cdown) {
        logHandler(LERR, "Invalid pointers as parameters");
        return;
    }
    if (!(*enabled)) {
        logHandler(LWARN, "Dialogue box is not enabled, not drawing");
        return;
    }
...
void drawDialogueBoxes (DialogueBox* dboxes, const float* cdowns, size_t count) { // MIGHT have a memory leak idk
    logHandler(LWARN, "drawDialogueBoxes() must be used with caution");
