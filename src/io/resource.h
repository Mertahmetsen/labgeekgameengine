    ...
    switch (dbxf.version) {
        case (unsigned long)1:
            ...
            *enabled = dbxf.enabled;
            return dbox;
        default:
            traceFuncErr(__func__, TextFormat("Unsupported version: %d, returning empty value", dbxf.version));
...
DialogueBox loadDBFile (const char* path, float* lifetime, bool* enabled) {
    ...
    DBoxFile dbxf = {0};
    if (!data) {
        traceFuncErr(__func__, TextFormat("Couldn't load file %s, returning empty value", path));
        return (DialogueBox){0};
    }
    if (bytesRead != sizeof(DBoxFile)) {
        traceFuncErr(__func__, TextFormat("Invalid DBFile: %s (read %d bytes, expected %zu), returning empty value", path, bytesRead, sizeof(DBoxFile)));
        UnloadFileData(data);
        return (DialogueBox){0};
    }
